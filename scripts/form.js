...
function validate() {
    ...
    if (nameValidator() && emailValidator() && phoneValidator() && dateValidator() && fixoValidator() && RGValidator() && complementoValidator() && numeroresValidator() && lograValidator() && alturaValidator() && pesoValidator() && cepValidator() && ruaValidator() && bairroValidator() && cidadeValidator() && estadoValidator()) {
        document.getElementsByClassName('redirect')[0].style.display = "inline-block"
        const msg = document.getElementsByClassName('success_msg')[0]
